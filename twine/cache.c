...
spindle_cache_update(SPINDLE *spindle, const char *localname, struct spindle_strset_struct *set)
{
	...
	twine_logf(LOG_DEBUG, PLUGIN_NAME ": [%dms] add inbound references to changeset\n", gettimediffms(&start));
	...
	twine_logf(LOG_DEBUG, PLUGIN_NAME ": [%dms] store proxy\n", gettimediffms(&start));
	...
	twine_logf(LOG_DEBUG, PLUGIN_NAME ": [%dms] cache to S3\n", gettimediffms(&start));
